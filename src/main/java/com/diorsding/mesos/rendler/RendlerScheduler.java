...
public class RendlerScheduler implements Scheduler {
    ...
    public RendlerScheduler(ExecutorInfo executorCrawler, ExecutorInfo executorRender, int totalTasks, String url) {
    ...
    public void statusUpdate(SchedulerDriver driver, TaskStatus status) {
        if (status.getState() == TaskState.TASK_FINISHED || status.getState() == TaskState.TASK_LOST) {
            System.out.println("Status update: task " + status.getTaskId().getValue() + " has completed with state "
                    + status.getState());
            finishedTasks++;
            ...
            if (finishedTasks == totalTasks) {
                ...
                parentDir = "/vagrant"; // TODO: remove hack
